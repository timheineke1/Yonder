...
## 10. Yonder App Addendum (Trial Scope)

This section is the Yonder-specific overlay for applying this design safely to dense product surfaces (chat, map, listing sidebar, filters) without breaking information flow.

### Scope Guardrails
- Apply Vercel-inspired cleanup incrementally, not as a full visual reset.
- Preserve existing product IA and interaction order (chat flow, listing actions, unlock/report logic).
- Trial scope first: filter menu and mini listing sidebar only.
- If trial quality improves readability and confidence, roll patterns to other surfaces.

### Typography Constraints (Yonder)
- Keep one primary UI family (Geist) across app controls and body.
- Use at most these UI roles in app surfaces:
  - `title`
  - `body`
  - `secondary`
  - `label`
  - `labelUC` (section markers only)
  - `mono` (technical refs only)
- Do not introduce additional ad-hoc font sizes for chips/buttons.
- Chat target: max 4 variants (`title`/`body`/`secondary`/`label`).
- Listing + mini sidebar target: max 6-7 variants including utility roles.

### Color Constraints (Yonder)
- Default text hierarchy:
  - primary `INK`
  - muted `MID`
- Avoid stacking multiple muted grays in one small component.
- Use source colors only where they encode data provenance (zoning/PDM/land-use/cadastre/GIS).
- Keep decorative accent usage minimal; prefer semantic meaning over visual decoration.

### Spacing & Density Constraints
- Keep control density consistent inside a region:
  - chip/button/select/input should share one padding rhythm per cluster.
- Do not mix compact and roomy controls in the same filter row.
- Maintain consistent vertical rhythm between section headers and content blocks.

### Border / Elevation Constraints
- Prefer one neutral border token for most UI edges.
- Reserve stronger borders/tints for selected/active states only.
- Avoid introducing extra separators unless they carry structural meaning.

### Responsive Constraints
- On reduced widths, prevent data-source row wrapping from breaking readability:
  - truncate values
  - shorten status labels
  - preserve label legibility
- Keep filter controls readable and aligned under narrow map widths.

### Trial Acceptance Checklist
- Filter menu feels like one control system (no mixed chip families).
- Mini listing sidebar reads cleanly at reduced width.
- Data-source colors map consistently across listing/sidebar and chat analysis pipeline.
- No regression to chat clarity.

### Rollback Note
- Snapshot before this trial is saved as:
  - `DESIGN.backup.pre-yonder-trial.md`
