...
function PlotRecapCard({ data }) {
  ...
  return (
    <div style={{ fontFamily: FF, background: WHITE, borderRadius: 10, overflow: "hidden", color: INK, border: `1px solid ${LIGHTER}` }}>
      <div style={{ padding: "12px 14px", borderBottom: `1px solid ${LIGHTER}`, background: BG2 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
          <span style={{ ...TC.labelUC, color: MID }}>Report</span>
          <span
            style={{
              ...TC.label,
              fontWeight: 600,
              color: isBuildable ? GREEN : "#9b1c1c",
              padding: "2px 8px",
              borderRadius: 99,
              border: `1px solid ${isBuildable ? `${GREEN}35` : "rgba(155,28,28,0.28)"}`,
              background: isBuildable ? `${GREEN}10` : "rgba(155,28,28,0.08)",
              letterSpacing: 0,
            }}
          >
            {data.verdictLabel}
        ...
        <div style={{ ...TC.title, color: INK, marginBottom: 4 }}>{data.location}</div>
        <div style={{ ...TC.secondary, color: MID, marginBottom: 6 }}>{data.zoningShort}</div>
        <div style={{ ...TC.body, color: INK }}>{data.verdictSummary}</div>
      </div>
      <div style={{ padding: "10px 12px 0" }}>
        <div style={{ display: "flex", gap: 4 }}>
          {data.keyFacts.map((f, i) => (
            <div key={i} style={{ flex: 1, background: BG2, borderRadius: 6, padding: "7px 8px", border: `1px solid ${LIGHTER}` }}>
              <div style={{ ...TC.labelUC, color: MID, marginBottom: 2 }}>{f.label}</div>
...
function ListingInsightSidebar({
  ...
  return (
    <div className="absolute top-0 right-0 bottom-0 w-[340px] bg-background border-l border-border z-[65] flex flex-col" style={{animation:"slideInRight 0.2s ease both",boxShadow:"-4px 0 20px rgba(0,0,0,0.06)"}}>

      {/* Header */}
      <div className="h-12 px-3 border-b border-border flex items-center gap-2 flex-shrink-0">
        <div className="flex-1 min-w-0">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-0.5">{pinPlot ? "Map pin" : plot.ref || cid}</div>
          <div className="text-sm font-medium text-foreground truncate">{plot.name}</div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {plot.score != null && <ScoreRing pct={plot.score} size={26} />}
          <button type="button" onClick={onClose} className="w-6 h-6 rounded-full border border-border bg-muted text-muted-foreground cursor-pointer flex items-center justify-center text-xs hover:text-foreground transition-colors">✕</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="relative w-full h-36 bg-muted overflow-hidden">
          ...
          <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center">
            <span className="text-[10px] text-white/80 drop-shadow">From portal</span>
            {plot.timeOnMarket && <span className="text-[10px] text-white/80 drop-shadow">{plot.timeOnMarket}</span>}
          </div>
          {!pinPlot && (
            <div className="absolute top-2 left-2">
              <span className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{background:v.bg,color:v.text||INK}}>{v.label}</span>
            </div>
          )}
        </div>
        ...
        <div className="px-3 py-3 border-b border-border">
          <div className="flex justify-between items-baseline mb-1.5">
            <span className="text-base font-medium text-foreground">{plot.price}</span>
            <span className="text-xs text-muted-foreground">{plot.area} · {plot.pricePerSqm}</span>
          </div>
          {plot.tag && <div className="text-xs text-muted-foreground mb-1.5">{plot.tag}</div>}
          <div className="text-xs text-foreground"><span className="text-muted-foreground">Region · </span>{plot.region || "—"}</div>
          <div className="text-xs text-foreground mt-1"><span className="text-muted-foreground">Type · </span>{plot.type || "—"}</div>

          {!pinPlot && Array.isArray(plot.amenities) && plot.amenities.length > 0 && (
            <div className="mt-2.5 pt-2.5 border-t border-border">
              <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1.5">Nearby</div>
          ...
          {!pinPlot && (
            <button type="button" onClick={() => onOpenListing(cid)} className="w-full mt-2.5 bg-transparent border border-border rounded-full py-1.5 text-xs text-muted-foreground cursor-pointer hover:text-foreground hover:border-foreground/30 transition-colors">
              View full listing →
            </button>
          )}
        </div>
        ...
        {!pinPlot && onRequestListingLocation && (
          <div className="px-3 py-3 border-b border-border">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-2">Location check</div>
            <p className="text-xs text-muted-foreground mb-2 leading-relaxed">Request official location confirmation with the realtor, or run the report now.</p>
            <button type="button" onClick={onRequestListingLocation} className="w-full bg-orange-500 border-none rounded-full py-2 text-xs font-medium text-white cursor-pointer mb-1.5">
              Request official location · €89 →
            </button>
            <button type="button" onClick={startAnalysis} className="w-full bg-transparent border border-dashed border-border rounded-full py-1.5 text-xs text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
              Run full report now
            </button>
          </div>
        )}
        ...
        <div className="px-3 py-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {!upgraded && !pinPlot && (
              <button type="button" onClick={onUpgrade} className="text-[10px] font-medium text-foreground bg-muted border border-border rounded-full px-2.5 py-0.5 cursor-pointer">Pro</button>
            )}
          </div>
          {pinPlot ? (
            <>
              <p className="text-xs text-muted-foreground mb-2">Pick a listing on the map for a full report.</p>
              <button type="button" onClick={() => onSendToChat(plot, { mode: "ask" })} className="w-full bg-foreground border-none rounded-full py-2 text-xs font-medium text-background cursor-pointer">
            ...
            <>
              {!recap && (
                <button type="button" onClick={startAnalysis} className="w-full bg-orange-500 border-none rounded-full py-2 text-xs font-medium text-white cursor-pointer">
              ...
              {recap && (
                <div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-xs font-medium text-foreground">Saved report</span>
                  </div>
                  <div className="relative">
                    <PlotRecapCard data={recap} />
                    {!upgraded && (
                      <button type="button" onClick={onUpgrade} className="absolute inset-0 border-none rounded-lg cursor-pointer text-xs font-medium text-blue-600" style={{background:"rgba(255,255,255,0.8)",backdropFilter:"blur(4px)"}}>
                        Upgrade to read full report
                      </button>
                    )}
                  </div>
                  <button type="button" onClick={() => onSendToChat(plot, { mode: "deep", recap })} className="w-full mt-2 bg-orange-50 border border-orange-200 rounded-full py-1.5 text-xs font-medium text-orange-600 cursor-pointer">
                    Chat →
                  </button>
                  <button type="button" onClick={startAnalysis} className="w-full mt-1.5 bg-transparent border border-border rounded-full py-1.5 text-xs text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
      ...
      <div className="px-3 py-3 border-t border-border flex-shrink-0">
        <button type="button" onClick={() => onAddToProject(cid)}
          className={`w-full rounded-full py-2 text-xs font-medium cursor-pointer border transition-colors ${inProject?"bg-green-50 border-green-200 text-green-700":"bg-foreground border-transparent text-background"}`}>
          {pinPlot ? (inProject ? "✓ Pin saved" : "+ Save pin") : (inProject ? "✓ Saved to project" : "+ Save to project")}
        </button>
        <p className="text-[10px] text-muted-foreground text-center mt-1.5">{inProject ? "Saved in Projects." : "Save this listing to Projects."}</p>
      </div>
...
function PlotListingPage({plotId,upgraded,onUpgrade,onBack,onAddToProject,inProject,savedAiRecap}){
  ...
  return(
    <div className="flex-1 bg-background flex flex-col overflow-hidden">
      ...
      {/* Top bar */}
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border h-12 px-6 flex items-center gap-3 flex-shrink-0">
        <button type="button" onClick={onBack} className="flex items-center gap-1.5 bg-transparent border border-border rounded-full px-3 py-1.5 cursor-pointer text-xs text-muted-foreground hover:text-foreground transition-colors">← Back</button>
        <span className="text-xs font-mono text-muted-foreground">{plot.ref}</span>
        <div className="ml-auto flex gap-2">
          <button type="button" onClick={()=>onAddToProject(plot.id)}
            className={`rounded-full px-3 py-1.5 text-xs font-medium cursor-pointer border transition-colors ${inProject?"bg-green-50 border-green-200 text-green-700":"bg-transparent border-border text-muted-foreground hover:text-foreground"}`}>
            {inProject?"✓ Saved":"+ Save to pipeline"}
          </button>
          <button type="button" className="bg-foreground border-none rounded-full px-3 py-1.5 text-xs font-medium text-background cursor-pointer">Contact agent →</button>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto">
        <div className="max-w-[860px] mx-auto w-full pb-20">

          {/* Gallery */}
          <div className="grid gap-px h-[400px]" style={{gridTemplateColumns:"1fr 1fr",gridTemplateRows:"1fr 1fr"}}>
            <div className="row-span-2 bg-muted relative overflow-hidden cursor-pointer" onClick={()=>setActiveImg(0)}>
              <PlotImage plot={plot} type={plot.type} index={0} style={{width:"100%",height:"100%"}}/>
              <div className="absolute top-3 left-3">
                <span className="text-xs font-medium px-2.5 py-1 rounded-full text-white" style={{background:"rgba(0,0,0,0.55)",backdropFilter:"blur(6px)"}}>{v.label}</span>
              </div>
            </div>
            {[1,2,3,4].map(i=>(
              <div key={i} className="bg-muted overflow-hidden cursor-pointer relative" style={{opacity:i>=plot.images.length?0.35:1}} onClick={()=>setActiveImg(i)}>
                <PlotImage plot={plot} type={plot.type} index={i} style={{width:"100%",height:"100%"}}/>
                {i===3&&plot.images.length>4&&(
                  <div className="absolute inset-0 bg-black/45 flex items-center justify-center text-xs font-medium text-white">+{plot.images.length-4} more</div>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="px-6 pt-5">
            ...
            <div className="flex items-start justify-between gap-4 mb-5 pb-5 border-b border-border">
              <div className="min-w-0">
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{plot.region}</div>
                <div className="text-xl font-medium text-foreground mb-1.5">{plot.name}</div>
                <div className="text-xs text-muted-foreground">{plot.tag}</div>
              </div>
              <div className="text-right flex-shrink-0">
                <div className="text-xl font-medium text-foreground">{plot.price}</div>
                <div className="text-xs font-mono text-muted-foreground mt-1">{plot.pricePerSqm}</div>
                <div className="text-xs font-medium text-foreground mt-1.5">{v.label} · {plot.score}/100</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              {[
                ...
                <div key={label} className="p-3 bg-muted/50 border border-border rounded-xl text-center">
            ...
            <div className="mb-6 p-4 bg-muted/50 border border-border rounded-xl">
              <div className="text-sm font-medium text-foreground mb-1.5">Pin not exact?</div>
              <p className="text-xs text-muted-foreground mb-3 leading-relaxed">We contact the agent first, then run your report — avoids paying twice.</p>
              {locationRequested ? (
                <div className="text-xs font-medium text-green-700 text-center py-2.5 bg-green-50 rounded-lg border border-green-200">✓ Request received — we’ll follow up within one business day (demo)</div>
              ) : (
                <button type="button" onClick={()=>setLocationRequested(true)} className="w-full bg-foreground border-none rounded-full py-2.5 text-xs font-medium text-background cursor-pointer">
            ...
            {savedAiRecap && (
              <div className="mb-6 pb-5 border-b border-border">
                <div className="text-sm font-medium text-foreground mb-3">AI land report · recap</div>
            ...
            <div className="mb-6 pb-6 border-b border-border">
              <div className="text-sm font-medium text-foreground mb-3">About this plot</div>
              <div className="relative overflow-hidden" style={{maxHeight:descExpanded?"none":"96px"}}>
                <p className="text-xs text-muted-foreground leading-relaxed">{plot.description}</p>
                {!descExpanded&&<div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-background to-transparent"/>}
              </div>
              <button type="button" onClick={()=>setDescExpanded(x=>!x)} className="mt-2.5 bg-transparent border-none p-0 cursor-pointer text-xs text-blue-600 font-medium">
            ...
            <div className="mb-5 pb-5 border-b border-border">
              <div className="text-sm font-medium text-foreground mb-3">Highlights</div>
              <div className="grid grid-cols-2">
                {plot.highlights.map((h,i)=>(
                  <div key={i} className="flex items-center gap-2 py-2 text-xs text-foreground border-b border-border">
                    <span className="text-green-500 flex-shrink-0 text-[8px]">●</span>{h}
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Amenities */}
            <div className="mb-5 pb-5 border-b border-border">
              <div className="text-sm font-medium text-foreground mb-3">In the area</div>
              <div className="grid grid-cols-3 gap-2">
                {(plot.amenities||[]).map((a,i)=>(
                  <div key={i} className="flex items-center gap-2 p-3 bg-muted/50 border border-border rounded-xl">
                    <span className="flex-shrink-0 leading-none">{a.icon}</span>
                    <div className="min-w-0">
                      <div className="text-xs font-medium text-foreground truncate">{a.label}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">{a.dist}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-muted/50 border border-border rounded-xl p-4 mb-5">
              <div className="text-sm font-medium text-foreground mb-3">Listed by</div>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-lg bg-background border border-border flex items-center justify-center text-sm flex-shrink-0">⊞</div>
                <div>
                  <div className="text-xs font-medium text-foreground">{plot.contact.name}</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{plot.contact.role}</div>
                </div>
              </div>
              <div className="flex gap-2">
                <a href={`tel:${plot.contact.phone}`} className="flex-1 bg-background border border-border rounded-full py-2 text-center text-xs font-medium text-foreground no-underline">Call</a>
                <a href={`mailto:${plot.contact.email}`} className="flex-[2] bg-foreground rounded-full py-2 text-center text-xs font-medium text-background no-underline">Send enquiry →</a>
              </div>
            </div>

            {/* Plot data */}
            <div className="mb-5">
              <div className="text-sm font-medium text-foreground mb-1">More from the listing</div>
              <p className="text-xs text-muted-foreground mb-4">Portal text; verified fields are in the checklist above after a report.</p>

              <div className="mb-3">
                {freeKeys.filter(k=>plot.technical[k]).map(k=>(
                  <div key={k} className="flex justify-between items-baseline py-1.5 border-b border-border">
                    <span className="text-[10px] uppercase tracking-wide text-muted-foreground">{k}</span>
                    <span className="text-xs font-medium text-foreground text-right max-w-[60%]">{plot.technical[k]}</span>
                  </div>
                ))}
              </div>

              {[
                {title:"General zoning rules",rows:[["PDM zone",plot.technical["PDM zone"]],["Land use","Rustic — Espaço Agrícola"],["Reclassification","No active process"],["Protected areas","None on this parcel"]]},
                ...
                {title:"Plot analysis / land use",rows:[["Land use","Rustic — Espaço Agrícola"],["RAN",plot.technical["RAN"]],["REN",plot.technical["REN"]],["Reclassification","No active process"]]},
                ...
                {title:"GIS layers · constraints",rows:[["Fire risk",plot.technical["Fire risk"]||"—"],["Grid distance",plot.technical["Grid distance"]||"—"],["Access",plot.technical["Access"]||"Paved road"],["Slope",plot.technical["Slope"]]]},
              ].map(({title,rows})=>(
                <div key={title} className="mb-2.5 rounded-xl border border-border overflow-hidden">
                  <div className={`px-3 py-2 flex items-center justify-between border-b border-border ${upgraded?"bg-background":"bg-muted/50"}`}>
                    <span className={`text-xs font-medium ${upgraded?"text-foreground":"text-muted-foreground"}`}>{title}</span>
                    {!upgraded&&<span className="text-[10px] font-medium bg-background border border-border rounded-full px-2 py-0.5">Pro</span>}
                  </div>
                  <div className="px-3" style={{filter:upgraded?"none":"blur(4px)",userSelect:upgraded?"auto":"none",pointerEvents:upgraded?"auto":"none"}}>
                    {rows.filter(([,v])=>v).map(([k,val])=>(
                      <div key={k} className="flex justify-between items-baseline py-1.5 border-b border-border last:border-0">
                        <span className="text-xs text-muted-foreground">{k}</span>
                        <span className="text-xs font-medium text-foreground text-right max-w-[55%]">{val}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {!upgraded&&(
                <div className="border border-border rounded-xl p-4 text-center bg-muted/50 mt-1">
                  <div className="text-xs font-medium text-foreground mb-1">Unlock full plot data</div>
                  <p className="text-xs text-muted-foreground mb-3 max-w-[240px] mx-auto leading-relaxed">PDM, cadastre, RAN/REN, registry — one report.</p>
                  <button type="button" onClick={onUpgrade} className="bg-foreground border-none rounded-full px-6 py-2 text-xs font-medium text-background cursor-pointer">Upgrade to Pro →</button>
                </div>
              )}
            </div>
            ...
            {upgraded&&(
              <div className="bg-muted/50 border border-border rounded-xl p-4 mb-5">
                <div className="text-sm font-medium text-foreground mb-1.5">Legal check</div>
                <p className="text-xs text-muted-foreground mb-3 leading-relaxed">Title verification, BUPI, PDM compliance and licencing pathway by Diogo Leal.</p>
                {contacted?(
                  <div className="text-xs font-medium text-green-700 text-center py-2.5 bg-green-50 rounded-lg border border-green-200">✓ Request sent — Diogo responds within 48h</div>
                ):(
                  <button type="button" onClick={()=>setContacted(true)} className="w-full bg-foreground border-none rounded-full py-2.5 text-xs font-medium text-background cursor-pointer">Request legal check →</button>
                )}
              </div>
            )}
    ...
    <div className="flex-shrink-0 bg-background border-b border-border">
      ...
      <div className="px-3 py-2 flex flex-wrap items-center gap-2">
        <select
          ...
          className="border border-border rounded-lg px-3 py-1.5 text-xs text-foreground bg-background cursor-pointer outline-none flex-shrink-0"
        >
        ...
        <button type="button" onClick={() => setPdmOpen((v) => !v)}
          className={`flex items-center gap-2 border border-border rounded-lg px-3 py-1.5 text-xs cursor-pointer flex-shrink-0 transition-colors ${pdmOpen?"bg-muted text-foreground":"bg-background text-foreground"}`}>
          <span>Zoning / PDM</span>
          <span className="text-muted-foreground">{(f.pdmTags||[]).length > 0 ? (f.pdmTags||[]).length : "Optional"} {pdmOpen?"▴":"▾"}</span>
        </button>

        <button type="button" onClick={() => setMoreOpen((v) => !v)}
          className={`border border-border rounded-lg px-3 py-1.5 text-xs cursor-pointer flex-shrink-0 transition-colors ${moreOpen?"bg-muted text-foreground":"bg-background text-muted-foreground"}`}>
          {moreOpen ? "▲ Fewer filters" : "▼ More filters"}
        </button>

        <button type="button" onClick={clearAll} className="bg-transparent border-none px-2 py-1.5 text-xs text-blue-600 cursor-pointer font-medium flex-shrink-0">
          Clear all
        </button>

        <span className="text-xs font-mono text-muted-foreground whitespace-nowrap flex-shrink-0 ml-auto">
          {resultCount}{totalCount != null && totalCount !== resultCount ? ` / ${totalCount}` : ""}
        </span>
      </div>

      {pdmOpen && (
        <div className="px-3 pb-3 border-t border-border">
          <p className="text-xs text-muted-foreground my-2 leading-relaxed" title="Municipal plan land-use classes (RJIGT-style). Dashed border = suggested for your current Use filters.">
            Zoning / PDM classes.
          </p>
          <div className="max-h-44 overflow-y-auto p-2 border border-border rounded-lg bg-muted/30">
            {["rustic", "urban"].map((group) => {
              ...
              return (
                <div key={group} className={keys.length ? "mb-2.5" : ""}>
                  <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mb-1.5">{title}</span>
                  <div className="flex flex-wrap gap-1">
                    {keys.map((key) => {
                      ...
                      return (
                        <button type="button" key={key} onClick={() => togglePdmTag(key)} title={key}
                          className={`px-2.5 py-0.5 rounded-full text-xs cursor-pointer transition-colors ${on?"bg-blue-50 border-blue-400 text-blue-600 border":"text-muted-foreground border"} ${sug&&!on?"border-dashed border-blue-300":"border-border"}`}>
                          {meta.label}
                        </button>
                      );
                    })}
      ...
      {moreOpen && (
        <div className="px-3 pb-3 border-t border-border flex flex-col gap-3">
          <div className="grid gap-3 mt-3" style={{gridTemplateColumns:"repeat(auto-fill, minmax(150px, 1fr))"}}>
            <div>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mb-1.5">Restrictions</span>
            ...
            <div>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mb-1.5">Land type</span>
              <Segmented options={[["all","All"],["rustic","Rustic"],["urban","Urban"],["mixed","Mixed"]]} value={f.landType||"all"} onChange={(v)=>patch({landType:v})}/>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mt-2.5 mb-1.5">Ruin</span>
              <Segmented options={[["all","All"],["has","Has ruin"],["none","No ruin"]]} value={f.ruin||"all"} onChange={(v)=>patch({ruin:v})}/>
            </div>
            <div>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mb-1.5">Price (€)</span>
              <div className="flex gap-2">
                <input type="text" inputMode="numeric" placeholder="Min" value={f.priceMin||""} onChange={(e)=>patch({priceMin:e.target.value})} className="flex-1 border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-background outline-none"/>
                <input type="text" inputMode="numeric" placeholder="Max" value={f.priceMax||""} onChange={(e)=>patch({priceMax:e.target.value})} className="flex-1 border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-background outline-none"/>
              </div>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mt-2.5 mb-1.5">Size (m²)</span>
              <div className="flex gap-2">
                <input type="text" inputMode="numeric" placeholder="Min" value={f.areaMin||""} onChange={(e)=>patch({areaMin:e.target.value})} className="flex-1 border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-background outline-none"/>
                <input type="text" inputMode="numeric" placeholder="Max" value={f.areaMax||""} onChange={(e)=>patch({areaMax:e.target.value})} className="flex-1 border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-background outline-none"/>
              </div>
            </div>
            <div>
              <span className="text-[10px] uppercase tracking-wide text-muted-foreground block mb-1.5">Sort</span>
              <select value={f.sortBy||"match"} onChange={(e)=>patch({sortBy:e.target.value})} className="w-full border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-muted/30 outline-none cursor-pointer">
          ...
          <div className="flex flex-wrap gap-3 items-end pt-2 border-t border-border">
            {[
              ...
              <div key={blk.key}>
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{blk.label}</div>
                <div className="flex gap-1 flex-wrap">
                  {blk.options.map((o)=>(
                    <button type="button" key={o} onClick={()=>patch({[blk.key]:f[blk.key]===o&&o!=="Any"?"Any":o})}
                      className={`rounded-full px-2.5 py-0.5 text-xs cursor-pointer border transition-colors ${f[blk.key]===o?"bg-foreground border-foreground text-background":"bg-transparent border-border text-muted-foreground hover:text-foreground"}`}>
                      {o}
                    </button>
                  ))}
                </div>
...
function InputBar({input,setInput,send,activePlot,activeICP,analysisState,runAnalysis,onRunAreaScan,hasDrawArea,drawAreaCount,drawAreaLens,onOpenListing,onAddToProject,onClearPlot,onBrowseCadastre,projectPlots,upgraded,requestUpgrade,onRequestListingLocation,isChatEmpty=false}){
  ...
  return(
    <div className="px-3 pb-3 pt-0 border-t border-border flex-shrink-0 bg-background">

      {/* Status hint — shown when plot is selected */}
      {activePlot&&(
        <div className="flex items-center gap-1.5 py-2" style={{animation:"fadeIn 0.2s ease both"}}>
          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{background:pinContext?ORANGE:GREEN_BRIGHT}}/>
          <span className="text-xs text-muted-foreground">
            {pinContext ? (
              <>Map pin — <span className="text-foreground font-medium">ask in chat</span> or use <span className="text-foreground font-medium">+</span> for save / image / cadastre</>
            ) : (
              <><span className="text-foreground font-medium">{activeListingId}</span> · Chat or <span className="text-foreground font-medium">+</span></>
            )}
          </span>
        </div>
      )}
      ...
      {activePlot && !pinContext && onRequestListingLocation && (
        <div className="pb-2 text-xs text-muted-foreground leading-relaxed" style={{animation:"fadeIn 0.2s ease both"}}>
          Approximate pin?{" "}
          <button type="button" onClick={onRequestListingLocation} className="border-none bg-transparent p-0 text-blue-600 font-medium cursor-pointer text-xs underline underline-offset-2">
      ...
      <div className="bg-muted/40 rounded-xl border border-border px-3 pt-2.5 pb-2 flex flex-col gap-2 min-h-[72px] justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {activePlot&&(
            <div className="relative flex-shrink-0">
              <div
                className="w-8 h-8 rounded-md overflow-hidden relative"
                style={{border:`1.5px solid ${pinContext?`${ORANGE}55`:"rgba(44,95,45,0.3)"}`,animation:"fadeIn 0.18s ease both"}}
                onMouseEnter={e=>{setTooltip(true);const b=e.currentTarget.querySelector("button");if(b)(b as HTMLElement).style.opacity="1";}}
                onMouseLeave={e=>{setTooltip(false);const b=e.currentTarget.querySelector("button");if(b)(b as HTMLElement).style.opacity="0";}}>
                <PlotImage plot={activePlot} type={activePlot.type} index={0}/>
                <button onClick={e=>{e.stopPropagation();onClearPlot();}}
                  className="absolute inset-0 w-full h-full bg-black/55 border-none cursor-pointer text-white text-xs flex items-center justify-center transition-opacity" style={{opacity:0}}>✕</button>
              </div>
              {tooltip&&(
                <div className="absolute bottom-9 left-0 bg-foreground text-background rounded-lg px-3 py-2.5 shadow-xl z-[300] min-w-[160px] pointer-events-none" style={{animation:"fadeIn 0.12s ease both"}}>
                  <div className="text-[10px] text-white/40 mb-0.5">{activeListingId}</div>
                  <div className="text-xs font-medium text-white mb-0.5">{activePlot.region}</div>
                  <div className="text-[10px] text-white/50">{activePlot.area} · {activePlot.price}</div>
                </div>
              )}
            </div>
          )}
          ...
          <input ref={fileInputRef} type="file" accept="image/*,.pdf,application/pdf" className="hidden"
            onChange={e=>{
              ...
              let tag;
              if(intent==="visual"&&f.type.startsWith("image/")){tag=`[Image — prefab render, hand-drawn home sketch, or inspiration photo: find visually similar listings & suggest relevant reading / blogs (demo) · ${f.name}]`;}
              else if(f.type.startsWith("image/")){tag=`[Image attachment (demo) · ${f.name}]`;}
              else{tag=`[PDF / document (demo) · ${f.name}]`;}
              setInput(s=>`${s}${s&&!s.endsWith(" ")?" ":""}${tag}`);
            }}/>
          <button type="button" title="Add image or PDF"
            onClick={()=>openFilePicker("visual")}
            className="w-7 h-7 rounded-md bg-transparent border border-border cursor-pointer flex items-center justify-center flex-shrink-0 text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors">
          ...
          <div className="relative flex-shrink-0" ref={plusRef}>
            <button onClick={()=>setPlusOpen(v=>!v)}
              className={`w-7 h-7 rounded-md border cursor-pointer flex items-center justify-center transition-all ${plusOpen?"bg-foreground border-foreground text-background":"bg-transparent border-border text-muted-foreground hover:border-foreground/30 hover:text-foreground"}`}>
            ...
            {plusOpen&&(
              <div className="absolute bottom-9 left-0 bg-background border border-border rounded-xl p-1.5 shadow-xl z-[200] min-w-[220px]" style={{animation:"fadeIn 0.15s ease both"}}>
                {activePlot&&(
                  <div className="px-2.5 py-2 border-b border-border mb-1">
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-0.5">Selected plot</div>
                    <div className="text-xs font-medium text-foreground">{activePlot.id} · {activePlot.region}</div>
                  </div>
                )}
                {!activePlot&&(
                  <div className="px-2.5 py-2 border-b border-border mb-1">
                    <div className="text-xs text-muted-foreground leading-relaxed">
                      {hasDrawArea ? <><strong className="text-foreground">Area</strong> · <strong className="text-foreground">+</strong> to scan</> : <>Drop pin & draw area on map. <strong className="text-foreground">+</strong> for cadastre & images.</>}
                    </div>
                  </div>
                )}
                  ...
                  item.divider
                    ...
                    : (
                      <div key={i} onClick={item.disabled?undefined:item.fn}
                        className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs text-foreground transition-colors ${item.disabled?"opacity-40 cursor-default":"cursor-pointer hover:bg-muted"}`}>
                        <span className={`w-4 text-center flex-shrink-0 ${item.highlight?"text-blue-600":"text-muted-foreground"}`}>{item.icon}</span>
                        <div className="flex-1 min-w-0">
                          <div className={`${item.highlight?"font-medium":""}`}>{item.label}</div>
                          {item.sub&&<div className="text-muted-foreground mt-0.5 leading-tight">{item.sub}</div>}
                        </div>
                        {item.upgrade&&<span className="text-[10px] text-blue-600 bg-blue-50 border border-blue-200 rounded-full px-2 py-px flex-shrink-0 font-medium">Upgrade</span>}
                      </div>
                    )
                )}
              </div>
          ...
          <button onClick={()=>input.trim()&&send(input.trim())}
            className={`w-7 h-7 border-none rounded-md cursor-pointer flex items-center justify-center flex-shrink-0 transition-colors ${input.trim()?"bg-foreground":"bg-border"}`}>
            <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
...
function ChatMapView({upgraded,requestUpgrade,onAddToProject,onCommitPlotsToPipeline,projectPlots,onGoToDashboard,onOpenListing,listingSavedScans,persistListingScan,pipelineFocusCanonicalId,onPipelineFocusConsumed,pipelineOnMapTick=0}){
  ...
  return(
    <div style={{flex:1,minHeight:0,display:"flex",overflow:"hidden"}}>
      ...
      {/* ── LEFT: CHAT ── */}
      <div className="flex flex-col h-full relative border-r border-border bg-background" style={{width:sidebarW,flexShrink:0}}>

        {/* Header */}
        <div className="h-12 px-4 border-b border-border flex items-center gap-2 flex-shrink-0 bg-background">
          <span className="flex-1 text-sm font-medium text-foreground">Yonder</span>
        ...
        {/* Messages */}
        <div className="ye-scroll flex-1 overflow-y-auto">
          <div className="p-3 flex flex-col gap-2">

          {/* Empty state */}
          {messages.length===0&&(
            <div style={{animation:"fadeIn 0.3s ease both"}}>
              <p className="text-xs uppercase tracking-wide text-muted-foreground mb-1.5 font-medium">Find land, understand anything about it.</p>
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium text-foreground cursor-pointer transition-colors ${chatEmptyBranch==="location"?"border-blue-500 bg-blue-50":"border-border bg-background"}`}
                >Location</button>
                <button
                  type="button"
                  onClick={()=>setChatEmptyBranch("use")}
                  className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium text-foreground cursor-pointer transition-colors ${chatEmptyBranch==="use"?"border-blue-500 bg-blue-50":"border-border bg-background"}`}
                >Use</button>
              </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {CHAT_ICPS.map((icp2)=>(
                    <button
                      ...
                      className="px-3 py-1.5 rounded-full border border-border bg-background text-xs text-foreground cursor-pointer hover:bg-muted transition-colors"
                    >{icp2.short}</button>
                  ))}
                  <button type="button" onClick={startPrefabIntent} title="Prefab & modular self-build land" className="px-3 py-1.5 rounded-full border border-border bg-background text-xs text-foreground cursor-pointer hover:bg-muted transition-colors">Prefab</button>
                </div>
              )}
            </div>
          )}
          ...
          {/* Chat messages */}
          {messages.map((msg,i)=>(
            <div key={i} className={`flex ${msg.role==="user"?"justify-end":"justify-start"}`} style={{animation:"fadeIn 0.2s ease both"}}>

              {msg.role==="assistant"&&msg.isThinking&&(
                <div className="flex gap-2 w-full items-start">
                  <div className="w-5 h-5 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-[10px] font-medium text-white leading-none">Y</span>
                  </div>
                  <div className="flex-1 bg-background border border-border rounded-xl px-3 py-2.5 flex flex-col gap-1.5">
                    {msg.steps.map((step,si)=>(
                      <div key={si} className="flex items-center gap-2" style={{animation:"fadeIn 0.2s ease both"}}>
                        <div className="w-1 h-1 rounded-full flex-shrink-0" style={{background:analysisStepColor(step),animation:`pulse 1.2s ease ${si*0.1}s infinite`}}/>
                        <span className="text-[10px] font-medium rounded-full px-1.5 py-px flex-shrink-0" style={{color:analysisStepColor(step),background:`${analysisStepColor(step)}18`,border:`1px solid ${analysisStepColor(step)}33`}}>
              ...
              {msg.role==="assistant"&&!msg.isThinking&&!msg.isVerdict&&!msg.isBulkResult&&(
                <div className="flex gap-2 w-full items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="bg-background border border-border rounded-xl px-3 py-2.5 text-sm text-foreground leading-relaxed">{msg.text}</div>
                    {msg.showResults&&(
                      <div className="mt-2 flex flex-col gap-1.5">
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <div className="w-1.5 h-1.5 rounded-full bg-blue-600 flex-shrink-0"/>
                          <span><span className="font-medium text-foreground">{msg.resultCount} listings</span> on the map — tap to open</span>
                        </div>
                        {mapResults.kind==="listings"&&mapResults.plots[0]&&(
                          <div onClick={()=>{ const p=mapResults.plots[0]; if(p){selectPlot(p); setTimeout(()=>runAnalysis(),300);} }}
                            ...
                            <div className="flex-1">
                              <div className="text-xs font-medium text-foreground">Run report (top match)</div>
                              <div className="text-xs text-muted-foreground mt-0.5">Sidebar · recap on listing</div>
                            </div>
                    ...
                    {msg.showUpgrade&&(
                      <button onClick={()=>requestUpgrade?.(null)} className="mt-1.5 bg-transparent border border-border rounded-full px-3 py-1 text-xs text-blue-600 cursor-pointer font-medium hover:border-blue-300 transition-colors">View plans →</button>
                    )}
                    {msg.showChips&&msg.icpId&&(()=>{
                      const ic=CHAT_ICPS.find(x=>x.id===msg.icpId);
                      return ic?(
                        <div className="mt-2 flex flex-col bg-background border border-border rounded-lg overflow-hidden">
                          {ic.prompts.map((pr,pi)=>(
                            <div key={pi} onClick={()=>send(pr)}
                              className={`px-3 py-2 flex items-center justify-between cursor-pointer text-xs text-foreground hover:bg-muted transition-colors ${pi<ic.prompts.length-1?"border-b border-border":""}`}>
                              <span>{pr}</span><span className="text-blue-600 flex-shrink-0 ml-2">→</span>
                            </div>
                          ))}
                        </div>
                      ):null;
              ...
              {msg.role==="user"&&(
                <div className="bg-foreground text-background rounded-xl px-3 py-2 text-sm max-w-[88%] leading-relaxed">{msg.text}</div>
              )}
            </div>
          ))}
          ...
          {/* Typing indicator */}
          {isTyping&&(
            <div className="flex gap-2 items-start" style={{animation:"fadeIn 0.2s ease both"}}>
              <div className="w-5 h-5 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0">
                <span className="text-[10px] font-medium text-white leading-none">Y</span>
              </div>
              <div className="bg-background border border-border rounded-xl px-3 py-2.5 flex gap-1 items-center">
                {[0,1,2].map(i=><div key={i} className="w-1 h-1 rounded-full bg-blue-600 opacity-40" style={{animation:`bounce 1s ease ${i*0.15}s infinite`}}/>)}
              </div>
            </div>
          )}
      ...
      <div style={{flex:1,display:isMobile&&mobileTab!=="map"?"none":"flex",flexDirection:"column",minWidth:0,paddingBottom:isMobile?52:0}}>
        {/* Toolbar */}
        <div className="h-12 border-b border-border bg-background flex items-center px-3 gap-2 flex-shrink-0">
          {/* Map / List toggle */}
          <div className="flex bg-muted/60 rounded-full p-0.5 border border-border">
            {[["map","Map"],["list","List"]].map(([m,lbl])=>(
              <button key={m} onClick={()=>setMapMode(m)}
                className={`rounded-full px-3.5 py-1 text-xs font-medium cursor-pointer border-none transition-all ${mapMode===m?"bg-foreground text-background":"bg-transparent text-muted-foreground hover:text-foreground"}`}
              >{lbl}</button>
            ))}
          </div>
          {/* Listings / Parcels toggle */}
          <div className="flex bg-muted/60 rounded-full p-0.5 border border-border">
            <button type="button" onClick={switchToListingsLens}
              className={`rounded-full px-3 py-1 text-xs font-medium cursor-pointer border-none transition-all ${!cadastreMode?"bg-foreground text-background":"bg-transparent text-muted-foreground hover:text-foreground"}`}>Listings</button>
            <button type="button" onClick={switchToParcelsLens}
              className={`rounded-full px-3 py-1 text-xs font-medium cursor-pointer border-none transition-all ${cadastreMode?"bg-foreground text-background":"bg-transparent text-muted-foreground hover:text-foreground"}`}>Parcels</button>
          </div>
          {hasDrawArea&&(
            <button type="button" onClick={()=>setMapMode("list")}
              className="text-xs font-medium bg-blue-50 border border-blue-200 rounded-full px-3 py-1 text-blue-600 cursor-pointer whitespace-nowrap">
        ...
        {activeProject ? (
          <>
            <button type="button" onClick={()=>{setActiveProject(null);setSelected([]);setDetailPlot(null);}}
              className="text-xs text-muted-foreground hover:text-foreground transition-colors bg-transparent border-none p-0 cursor-pointer">
              Projects
            </button>
          ...
          <span className="flex-1 text-sm font-medium text-foreground">Projects</span>
        )}
        <div className="flex items-center gap-2 ml-auto">
          {/* Inside project toolbar */}
          {activeProject&&(
            <>
              <div style={{position:"relative"}}>
          ...
          {!activeProject&&(
            <button type="button" onClick={()=>setShowNewProject(true)}
              className="bg-foreground text-background rounded-full px-4 text-sm font-medium cursor-pointer border-none" style={{padding:"7px 16px"}}>
              + New project
            </button>
          )}
        </div>
...
export default function YonderExplorerAppInner({
  ...
  skipLanding = false,
  sidebarMode,
}: { embed?: boolean; skipLanding?: boolean; sidebarMode?: "search" | "projects" }){
  const router = useRouter();
  const openAppFirst = embed || skipLanding;
  const [view, setView] = useState(openAppFirst ? "app" : "landing");
  // When sidebarMode is controlled externally (icon rail), map it to internal nav names
  const externalNav = sidebarMode === "projects" ? "Projects" : sidebarMode === "search" ? "Search" : null;
  const [activeNav,setActiveNav]=useState(externalNav ?? "Search");
  // Keep internal activeNav in sync when icon rail changes
  useEffect(()=>{ if(externalNav) setActiveNav(externalNav); },[externalNav]);
  ...
  return(
    ...
    >
      ...
      {!sidebarMode && <GlobalNav activeNav={activeNav} setActiveNav={v=>{setActiveNav(v);closeListing();}} upgraded={upgraded} onBackToLanding={goHome}/>}
